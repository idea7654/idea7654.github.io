...
- 시체의 경우엔 해당 플레이어가 갖고있던 총알을 즉시 획득한다.
...
- 중간에 죽게 되면 GameOver 표시와 함께 약 5초 뒤에 자동으로 로비로 돌아간다.
- 무기를 갖고있을 때, 3번을 눌러 무기를 장착/집어넣기를 할 수 있다.
- 무기를 장착하지 않고 있을 때 F버튼을 눌러 근접공격을 할 수 있다.
- 근접공격은 3단 콤보로 이루어져있으며, 일정 시간 내에 F버튼을 다시 누르면 다음 콤보가 실행되고 일정 시간이 지나면 다시 처음부터 콤보공격이 시작된다.
- 근접공격은 플레이어의 전방벡터 80cm이내에 적이 있으면 데미지를 입힌다.
- 맵의 몇가지의 엄폐물이 존재한다.
- 엄폐물은 실시간으로 시뮬레이션되며, 총알이나 근접공격으로 공격당할 경우 부서지게된다.
