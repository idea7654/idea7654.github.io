...
1. 무기를 장착한 상태에서는 무기를 장착하지 않은 상태와 다른 애니메이션이 적용된다. - 100%
2. 무기를 장착한 상태에서는 마우스의 좌클릭과 우클릭이 활성화되어 좌클릭으로 발사/우클릭으로 확대를 할 수 있다. - 100%
3. 체력이 0이 되면 사망 애니메이션이 재생되고, 컨트롤할 수 없게 된다. - 100%
4. 무기를 갖고있을 때, 3번을 눌러 무기를 장착/집어넣기를 할 수 있다. - 100%

### 작업 내용

1. 무기를 장착한 상태는 bool타입의 Equip Gun으로, 해당 값에 따라 각기 다른 애니메이션을 적용 - 100%
2. 조건은 1번과 같으며 우클릭을 누르고 누르고있을 때 확대를 하며 우클릭을 해제하면 원래대로 돌아간다 - 100%
3. 체력은 float타입의 HealthAmount변수이며 해당 값이 0.0f이하가 되면 사망 애니메이션을 재생하고, 컨트롤을 해제함 - 100%
4. 무기를 갖고있는 상태는 bool타입의 hasGun으로, 해당 값이 true일 때만 3번키를 눌러 장착/집어넣기를 수행 - 100%
...
### 해당 요구사항
12. 일반 몸통에 맞추면 20의 데미지가, 머리에 맞추면 2.5배인 50의 데미지가 들어간다. - 50%
15. 무기를 장착하지 않고 있을 때 F버튼을 눌러 근접공격을 할 수 있다. - 100%
16. 근접공격은 3단 콤보로 이루어져있으며, 일정 시간 내에 F버튼을 다시 누르면 다음 콤보가 실행되고 일정 시간이 지나면 다시 처음부터 콤보공격이 시작된다. - 100%
17. 근접공격은 플레이어의 전방벡터 80cm이내에 적이 있으면 데미지를 입힌다. - 100%
  
4. 로그인 화면, 회원가입 화면, 로비 화면, 인게임 화면 총 4개의 화면이 있다. - 100%
5. 로그인 화면에는 이메일과 패스워드를 입력할 수 있는 Input Component와 로그인을 수행하는 버튼, 회원가입 페이지로 넘어가는 버튼이 있다. - 100%
6. 회원가입 화면에는 이메일과 패스워드, 닉네임을 입력할 수 있는 Input Component와 회원가입을 수행하는 버튼, 로그인 페이지로 넘어가는 버튼이 있다. - 100%
7. 로비 화면에는 매칭을 시작하는 Matching Start버튼이 있다. - 100%
  
### 작업 내용
1. 플레이어 스켈레탈 메시에 새로운 콜리전채널을 넣어 총알에 맞았을 때 해당 콜리전채널과 반응을 했는지 안했는지로 구분한다. - 100%
2. EquipGun이 false일때 F를 누르면 공격 애니메이션 몽타주를 실행하도록 한다. - 100%
3. 현재 콤보는 CurrentCombo로, 최대콤보는 MaxCombo로 관리한다. MaxCombo는 3이며 CurrentCombo는 0으로, 근접공격을 수행할 때마다 MaxCombo가 1씩 올라가면서 해당 콤보에 맞는 애니메이션 몽타주가 재생된다. 또한, 마지막으로 재생한 근접공격으로부터 1.5초가 지나면 CurrentCombo를 0으로 초기화한다. - 100%
4. 애니메이션 몽타주가 실행될 때, 중간 지점에 AnimNotify를 넣어 해당 알림이 오게 되면 LineTracing을 캐릭터의 전방벡터로 80cm만큼 체크를 하여 유저가 있는지 확인하고, 있다면 데미지를 입힌다. - 100%

## 4주차

<figure class="video_container">
  <video controls="true" allowfullscreen="true" poster="./resource/keyEvent.png">
    <source src="./resource/4Week.mp4" type="video/mp4">
  </video>
</figure>
  
### 해당 요구사항
18. GameThread와 별도로 작동하는 스레드를 만들어 서버와의 통신을 가능하게 만든다. - 100%
19. 로그인 화면에서 로그인 버튼을 누르면 로그인 요청을 서버에 보낸 후 에러/성공 여부를 받아 성공일 경우 로비로 넘어가며 해당 게임의 유저 이름을 서버에서 받아온 닉네임으로 설정한다. - 100%
20. 회원가입 화면에서 회원가입 버튼을 누르면 회원가입 요청을 서버에 보낸 후 에러/성공 여부를 받아 화면에 표시한다. - 100%
21. 로그인 화면과 회원가입 화면에서 서로의 화면으로 넘어갈 수 있도록 한다. - 100%
22. 로비 화면에서 Matching Start버튼을 누르면 매칭 요청 패킷을 서버에 보내고 텍스트를 Matching Cancel로 변경한다. - 100%
23. 로비 화면에서 Matching Cancel버튼을 누르면 매칭 취소 패킷을 서버에 보내고 텍스트를 Matching Start로 변경한다. - 100%
24. 서버에서 정해진 인원만큼 매칭을 수행했을 때, 방을 하나 만들어 방에 해당하는 모든 유저가 게임을 시작한다. - 100%
  
### 작업 내용
1. 별도로 작동하는 스레드는 FRunnable로 구현한다. - 100%
2. FRunnable 스레드에서 패킷을 받고, 이를 게임로직에서 사용할 수 있도록 큐를 만들어 큐에 넣고 PlayerController에서 빼서 사용한다. - 100%
3. 이 때, 두 스레드에서 data race가 일어날 수 있으므로 데이터를 넣고 뺄 때 lock을 걸어 사용한다. - 100%
4. 에러/성공여부를 화면에 표시하는 기능은 WidgetAnimation을 사용하며 Opacity를 1로 만든 후 3초에 걸쳐 0으로 만든다. - 100%
5. 패킷은 Flatbuffers를 사용하며, Flatbuffers로 패킷을 조립하는 함수는 FRunnable에서 구현한다. - 100%
6. 게임을 시작하는 것은 서버에서 판단하며, 클라이언트는 서버에서 신호가 오면 게임을 시작시킨다. - 100%
  
## 5주차
  
<figure class="video_container">
  <video controls="true" allowfullscreen="true" poster="./resource/keyEvent.png">
    <source src="./resource/5Week.mp4" type="video/mp4">
  </video>
</figure>
  
### 해당 요구사항
  
25. 유저의 이동/점프/앉기를 동기화한다. - 100%
26. 유저가 맵에 존재하는 총을 소유하게 되면 이를 동기화한다. - 100%
27. 유저가 무기를 장착/변경하면 이를 동기화한다. - 100%
28. 유저가 총알을 발사하면 이를 동기화하고 데미지를 계산하여 서버로 보낸다. 데미지 계산연산은 발사한 클라이언트에서 담당하고, HP관리는 서버에서 담당한다. - 50%
  
1. 이동할 때마다 발소리 사운드가 재생된다. - 0%
  
### 작업 내용
  
1. 유저의 이동/점프/앉기, 총기 소유, 무기 장착/변경 동기화는 이벤트 기반으로 구현하며 상태가 바뀔때마다 패킷을 보낸다. - 100%
2. 데미지 계산은 총알을 발사한 클라이언트에서 담당한다. - 100%
...
28. 유저가 총알을 발사하면 이를 동기화하고 데미지를 계산하여 서버로 보낸다. 데미지 계산연산은 발사한 클라이언트에서 담당하고, HP관리는 서버에서 담당한다. - 100%
29. 유저가 근접공격을 하면 이를 동기화하고 데미지를 계산하여 서버로 보낸다. - 100%
30. 28,29의 과정에서 유저가 죽게되면 이를 해당 룸에 있는 모든 인원에게 보내 나머지 플레이어들에게는 킬로그를 제공&전체인원 UI의 숫자 줄이기를 수행한다. - 100%
31. 죽은 플레이어는 GameOver애니메이션을 재생하고 5초 뒤에 자동으로 로비로 돌아간다. - 100%
32. 30의 과정결과 한명만 남게 되면 남은 유저에게 패킷을 보내 승리 애니메이션을 재생하고, 5초 뒤에 자동으로 로비로 돌아간다. - 100%
  
1. 이동할 때마다 발소리 사운드가 재생된다. - 100%
  
### 작업 내용
  
1. 총알 발사 이펙트를 만든다. - 100%
2. 공격한 시점의 Current Combo를 보내어 각 Combo에 맞는 애니메이션을 재생한다. - 100%
3. 죽는 것은 서버가 판단하여 패킷을 보내고, 킬로그와 UI줄이기는 UI애니메이션, UI숫자 변경을 사용한다. - 100%
4. GameOver애니메이션을 재생할 때 유저의 상태를 Die로 만들어 컨트롤을 불가능하게 한다. - 100%
5. 승리 애니메이션을 재생할 때 컨트롤을 불가능하게 한다. - 100%
